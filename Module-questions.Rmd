...
Case (i.e. group of individuals as registered) corresponds to the unit used for assistance, as registered by UNHCR and also used during continuous assessment / protection monitoring. __It is recommended to have one form filled by each case rather than by each household. In this form one repeat questions can allow to records all linkeds cases with their respective CaseID__ for the following reasons:
 ...
 * The types of links between cases (level of sharing for accomodation and for food) that forms an household and the household profile can easily be collected. 
